...
private struct Triangle
{
    ...
    float[] vertices = [
         0.0,  0.5, 1.0, // ( 0.0,  0.5) white
         0.5, -0.5, 0.5, // ( 0.5, -0,5) gray
        -0.5, -0.5, 0.0  // (-0.5, -0.5) black
    ];
...
// shader sources
    ...
    void loadShaders()
    {
        ...
        load("source/grayshadedtriangle/vertexShader.vert", vertexSRC);
        load("source/grayshadedtriangle/fragmentShader.frag", fragmentSRC);
    }
}

///
struct GrayShadedTriangleApp
{
public:
///
    void run()
...
private:
    ...
    void initWindow()
    {
        ...
        window = glfwCreateWindow(800, 600, "Gray Shaded Triangle!", null, null);
    ...
    void initRender()
    {
        ...
        triangle.colAttrib = glGetAttribLocation(triangle.shaderProgram, "shade");
        glEnableVertexAttribArray(triangle.colAttrib);
        glVertexAttribPointer(triangle.colAttrib, 1, GL_FLOAT, GL_FALSE,
                              3 * float.sizeof, cast(void*)(2 * float.sizeof));
    }
